<?php
	...
	add_action('wbcr_factory_forms_445_plugin_created', 'wbcr_factory_forms_445_plugin_created');

	function wbcr_factory_forms_445_plugin_created($plugin)
	{
		$plugin->forms = new Wbcr_FactoryForms445_Manager($plugin);
	}

	if( !class_exists('Wbcr_FactoryForms445_Manager') ) {

		class Wbcr_FactoryForms445_Manager {
			 ...
			 *
			 * @see FactoryForms445_Form::registerControl()
			 *
			 * @since 1.0.0
			 * @return void
			 */
			 ...
			 *
			 * @see FactoryForms445_Form::registerHolder()
			 *
			 * @since 1.0.0
			 * @return void
			 */
			 ...
			 *
			 * @see FactoryForms445_Form::registerCustomElement()
			 *
			 * @since 1.0.0
			 * @return void
			 */
	...
	if( !class_exists('Wbcr_FactoryForms445_Form') ) {
		/**
		 * An abstraction for forms.
		 */
		class Wbcr_FactoryForms445_Form {
			 ...
			 *
			 * @since 1.0.0
			 * @var Wbcr_IFactoryForms445_ValueProvider
			 */
			private $provider;
			 ...
			 *
			 * @since 1.0.0
			 * @var Wbcr_FactoryForms445_FormLayout
			 */
			public $layout;
			 ...
			 */
			public function __construct(array $options, Wbcr_Factory448_Plugin $plugin)
			{
				// register controls once, when the first form is created
				if( !Wbcr_FactoryForms445_Manager::$controls_registered ) {

					do_action('wbcr_factory_forms_445_register_controls', $plugin);

					//if( !empty($plugin) ) {
					do_action('wbcr_factory_forms_445_register_controls_' . $plugin->getPluginName(), $plugin);
					//}

					Wbcr_FactoryForms445_Manager::$controls_registered = true;
				}
				...
				Wbcr_FactoryForms445_Manager::$temper = 'flat';
			}
			...
			{
				return apply_filters('wbcr_factory_448_form_items', $this->items, $this->name);
			}
			 ...
			 *
			 * @since 1.0.0
			 * @return Wbcr_FactoryForms445_Control[]
			 */
			 ...
			 * @return Wbcr_FactoryForms445_FormElement|null A form element.
			 */
			 ...
			 * @return Wbcr_FactoryForms445_FormElement[] Created elements.
			 */
			 ...
			 * @since 1.0.0
			 * @param array $item Item data.
			 * @return Wbcr_FactoryForms445_Control A control object.
			 */
			public function createControl($item)
			{
				...
				if( is_array($item) ) {
					...
					$control_data = Wbcr_FactoryForms445_Manager::$registered_controls[$item['type']];
			 ...
			 * @since 1.0.0
			 * @param array $item Item data.
			 * @return Wbcr_FactoryForms445_Holder A control holder object.
			 */
			public function createHolder($item)
			{
				...
				if( is_array($item) ) {
					...
					$holderData = Wbcr_FactoryForms445_Manager::$registered_holders[$item['type']];
			 ...
			 * @return Wbcr_FactoryForms445_FormElement A custom form element object.
			 */
			public function createCustomElement($item)
			{
				...
				if( is_array($item) ) {
					...
					$data = Wbcr_FactoryForms445_Manager::$registered_custom_elements[$item['type']];
			...
			public function html($options = array())
			{
				...
				if( !isset(Wbcr_FactoryForms445_Manager::$form_layouts[$this->form_layout]) ) {
					die(sprintf('[ERROR] The form layout %s was not found.', $this->form_layout));
				}

				// include a render code
				$layout_data = Wbcr_FactoryForms445_Manager::$form_layouts[$this->form_layout];
			...
			public static function connectAssetsForItem($item)
			{
				...
				if( self::isControl($type) ) {
					$haystack = Wbcr_FactoryForms445_Manager::$registered_controls;
				} elseif( self::isControlHolder($type) ) {
					$haystack = Wbcr_FactoryForms445_Manager::$registered_holders;
				}
			...
			{
				return isset(Wbcr_FactoryForms445_Manager::$registered_controls[$item['type']]);
			}
			...
			{
				return isset(Wbcr_FactoryForms445_Manager::$registered_holders[$item['type']]);
			}
			...
			{
				return isset(Wbcr_FactoryForms445_Manager::$registered_custom_elements[$item['type']]);
			}
		}
	}